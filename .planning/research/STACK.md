...
**Domain:** Svelte 5 component library — publishable npm package with shadcn-svelte CLI integration
**Researched:** 2026-03-23
**Confidence:** HIGH (verified against npm registry, official Svelte docs, shadcn-svelte docs)
...
The `package.json` must be configured precisely or bundlers silently fail. The correct pattern for a Svelte-only component library with no CSS:
...
The registry sits inside the same SvelteKit project that hosts the demo. Structure:
...
src/
  lib/
    ...
    wheel-picker/
      ...
      index.ts
static/
  r/                  ← generated by `registry:build`, served as static files
    wheel-picker.json
registry.json         ← defines what the CLI can install
```
...
```json
{
  ...
  "name": "svelte-wheel-picker",
  "homepage": "https://your-demo-url.com",
  "items": [
    {
      "name": "wheel-picker",
      "type": "registry:component",
      "title": "Wheel Picker",
      "description": "iOS-style wheel picker with inertia scrolling.",
      "files": [
        {
          "path": "./src/lib/wheel-picker/wheel-picker.svelte",
          "type": "registry:component"
        },
        {
          "path": "./src/lib/wheel-picker/wheel-picker-wrapper.svelte",
          "type": "registry:component"
        }
      ]
    }
...
Build with `npm run registry:build` → outputs to `static/r/`. The SvelteKit dev server automatically serves `static/` at the root, so `http://localhost:5173/r/wheel-picker.json` works immediately in dev.
...
npx sv create svelte-wheel-picker
...
# Testing stack
npm install -D vitest @vitest/browser vitest-browser-svelte
...
**For the registry (shadcn-svelte CLI integration):**
...
*Stack research for: Svelte 5 wheel picker component library*
*Researched: 2026-03-23*
